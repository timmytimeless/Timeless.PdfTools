...
public class AddBarcode39Instruction<TData> : PdfBuildInstruction<TData>
{
    private readonly string _barcodeValue;
    private readonly PdfLayoutRectangleMm _layoutRectangle;

    public AddBarcode39Instruction(PdfLayoutRectangleMm layoutRectangle, string barcodeValue)
    {
    ...
    public override byte[] Execute(byte[] pdfBytes, TData data)
    {
        ...
        using (var ms = new MemoryStream())
        {
            ...
            var image = barcode39.CreateImageWithBarcode(pdfStamper.GetOverContent(1), BaseColor.Black, BaseColor.Black);
